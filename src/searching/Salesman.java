...
public class Salesman {
	...
	// Constructor - Create an adjacency list by using an ArrayList of city node objects each with an ArrayList of routes
	Salesman(String[] cities) {
		graph_size = cities.length;
		city_list = new ArrayList<City>();
		
		// Initialize city_list with names of cities
	...
	void add_connection(int x, int y, int weight) {
		...
		City city_x = city_list.get(x); // Grab the city node from city_list
		city_x.add_route(y, weight); 
		city_list.set(x, city_x); // Put the node back into the list
	}
